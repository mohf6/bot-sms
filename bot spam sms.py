...
LIMIT = 30 #จำนวนสูงสุดที่ใส่ได้ต่อการยิง 1 ครั้ง
blacklist = [
    "",
    ""
...
@bot.event
async def on_ready():
   await bot.change_presence(status=discord.Status.idle, activity=discord.Activity(type=discord.ActivityType.playing, name='KAMUI COMMUNITY'))
...
async def help(ctx):
    ...
    embed.set_footer(text=" | Bot by KAMUI PRO")
    embed.set_author(name='KAMUI SMS')
    embed.set_thumbnail(url='https://i.pinimg.com/originals/70/a5/52/70a552e8e955049c8587b2d7606cd6a6.gif')
    embed.set_image(url='https://media.discordapp.net/attachments/955174361378226238/955475320813805648/kamui2.png')
...
async def sms(ctx, phone=None, amount=None):
    if (str(ctx.message.channel.id) == 'ให้ใส่ไอดีห้อง'):
        if (phone == None or amount == None):
            ...
            if (phone not in blacklist):
                try:
                    ...
                    embed.set_footer(text=" | KUY by KAMUI PRO")
                    embed.set_author(name='KAMUI SMS')
                    embed.set_thumbnail(url='https://media.discordapp.net/attachments/680449178626818065/909437371097972747/794404253744496642.gif')
                    embed.set_image(url='https://media.discordapp.net/attachments/955174361378226238/955475320813805648/kamui2.png')
                    await ctx.message.delete()
        ...
        embed=discord.Embed( description=":chart_with_downwards_trend: : ใส่ให้ถูกห้องไอควาย \n #〖📩〗-»《. ", color=0x00ff00, timestamp=datetime.utcnow())
        embed.set_footer(text=" | Bot by KAMUI PRO")
        embed.set_author(name='KAMUI SMS')
        embed.set_thumbnail(url='https://media.discordapp.net/attachments/680449178626818065/909437371097972747/794404253744496642.gif')
        embed.set_image(url='https://media.discordapp.net/attachments/955174361378226238/955475320813805648/kamui2.png')
